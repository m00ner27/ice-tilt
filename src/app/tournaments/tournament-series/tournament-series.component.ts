...
@Component({
  selector: 'app-tournament-series',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './tournament-series.component.html',
  styleUrl: './tournament-series.component.css'
})
export class TournamentSeriesComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.series$ = this.store.select(TournamentsSelectors.selectCurrentTournamentSeries);
  ...
  private loadSeries() {
    if (this.seriesId && this.bracketId) {
      this.store.dispatch(TournamentsActions.loadTournamentSeriesById({ 
  ...
  private refreshSeries() {
    console.log('Refreshing tournament series data...');
    this.loadSeries();
  }
